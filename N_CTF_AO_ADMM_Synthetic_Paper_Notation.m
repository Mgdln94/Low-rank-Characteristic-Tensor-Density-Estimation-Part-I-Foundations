function [A,lambda,Out] = N_CTF_AO_ADMM_Synthetic_Paper_Notation(Y,I,F,opts)
% input:
...
while(1)
    ...
    for n = 1:N     
        A{n} = least_squares_Aj_update(opts.marg,rows{n},cols{n},lambda,A,Y(rows{n}));
        fprintf('Updating A_%d, cost: %.30f \n',n, Loss_Coupled(Y,A,opts,lambda,nrm_sqr))
        %Loss_Coupled(Y,A,opts,lambda,nrm_sqr)
        GG{n} = A{n}'*A{n};
    end
     ...
     [lambda, u] = admm_lambda_update(G,V,lambda',u,N+1,opts,rho(2),max_iter);
     lambda = lambda';

     fprintf('Updating lambda, cost: %.30f \n', Loss_Coupled(Y,A,opts,lambda,nrm_sqr))
    ...
    if iter>100
    set(gca,'ylim',[10^(-50) Out.hist_cost(100)],'YLimMode','Manual');
    end
    if iter>1000
    set(gca,'ylim',[10^(-50) Out.hist_cost(1000)],'YLimMode','Manual');
    end
    plot(gca,1:iter, Out.hist_cost(1:iter), '-r','LineWidth',2)
    grid on;
